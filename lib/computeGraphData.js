...
function computeWindTurbineGraphs(p) {
  ...
  // Real Science: Heier's empirical Cp model combined with solidity & tip-loss
  const getCp = (lambda, pitch, B) => {
    if (lambda <= 0.1) return 0;
    
    // Solidity effect: optimal TSR shifts lower as blade count increases
    const lambdaOptB = 8.1 * Math.sqrt(3 / B);
    
    // Map the actual lambda to the Heier model's native optimal lambda (~8.1)
    const lambda_eff = lambda * (8.1 / lambdaOptB);
    
    // Base aerodynamic model (Heier)
    const lambda_i = 1 / (1 / (lambda_eff + 0.08 * pitch) - 0.035 / (Math.pow(pitch, 3) + 1));
    const cp_base = 0.5176 * (116 / lambda_i - 0.4 * pitch - 5) * Math.exp(-21 / lambda_i) + 0.0068 * lambda_eff;
    
    // Non-monotonic empirical blade efficiency (peaks at 3 blades)
    // Reflects tip losses (low B) vs wake/solidity interference (high B)
    const getBladeEfficiency = (n) => {
      if (n <= 1) return 0.75;
      if (n === 2) return 0.94;
      if (n === 3) return 1.00;
      if (n === 4) return 0.96;
      if (n === 5) return 0.92;
      if (n === 6) return 0.88;
      return Math.max(0.3, 0.88 - (n - 6) * 0.04);
    };
    
    return Math.max(0, cp_base * getBladeEfficiency(B));
  };

  // The controller maintains this optimal TSR below rated wind speeds
  const optimalTSR = 8.1 * Math.sqrt(3 / blades);
  const currentTSR = (currentV > 0) ? +(10 * R / (currentV * 60 / (2 * Math.PI))) : 0;
  
  // Power curve (assuming generator caps at a rated speed, but here we show aerodynamic potential)
  const vArr = linspace(0, 40, N);
  const powerData = vArr.map((v) => {
    // In a real turbine, controller maintains optimal TSR until rated power
    const lambda = optimalTSR; 
    const Cp = getCp(lambda, 0, blades);
    return {
      v: +v.toFixed(2),
      power: +(0.5 * rho * A * v * v * v * Cp / 1000).toFixed(2), // Actual power (kW)
      betz: +(0.5 * rho * A * v * v * v * 0.593 / 1000).toFixed(2), // Betz limit (kW)
    };
  });

  // TSR efficiency curve
  const tsrArr = linspace(1, 15, 60);
  const tsrData = tsrArr.map((tsr) => {
    return { tsr: +tsr.toFixed(2), Cp: +getCp(tsr, 0, blades).toFixed(4) };
  });

  // Blade stress vs wind speed based on BEM thrust
  const stressData = vArr.map((v) => {
    const CT = 0.8; // Typical thrust coefficient at optimal TSR
    const thrust = 0.5 * rho * A * v * v * CT; // Total thrust force (N)
    
    // Aerodynamic flapwise bending moment per blade (assuming center of pressure at 2/3 R)
    const M_flap = (thrust * (2/3 * R)) / blades; 
    
    // Root section modulus scaling law: Z ∝ R^3 (to maintain constant stress scaling with size)
    const Z = 1.2e-5 * Math.pow(R, 3); 
    const sigma = (M_flap / Z) / 1e6; // Stress in MPa
    
    return { v: +v.toFixed(2), stress: +sigma.toFixed(2) };
  });

  // Calculate current operating Cp based on actual current TSR
  const currentCp = getCp(currentTSR, 0, blades);

  return [
    {
      id: "power_curve", title: "Aerodynamic Power Curve", xKey: "v", xLabel: "Wind Speed (m/s)",
      yLabel: "Power (kW)", type: "line",
      paramKey: "Wind_Speed", paramMin: 0, paramMax: 40, paramDefault: 12, paramUnit: "m/s",
      series: [
        { key: "power", label: "Actual Aerodynamic Power", color: "#6366f1" },
      ...
      annotation: `Current aerodynamic output: ${(0.5 * rho * A * Math.pow(currentV, 3) * currentCp / 1000).toFixed(1)} kW`,
    },
    {
      id: "blade_stress", title: "Flapwise Root Bending Stress", xKey: "v", xLabel: "Wind Speed (m/s)",
      yLabel: "Stress (MPa)", type: "line",
      paramKey: "Wind_Speed", paramMin: 0, paramMax: 40, paramDefault: 12, paramUnit: "m/s",
      series: [{ key: "stress", label: "Root Bending Stress", color: "#f59e0b" }],
      data: stressData, currentX: currentV,
      regions: [{ x1: 25, x2: 40, color: "rgba(239,68,68,0.10)", label: "Fatigue Zone (≥250 MPa)" }],
      annotation: "Assumes standard composite scaling laws",
    },
    {
      id: "tsr_curve", title: "Power Coefficient (Cp) vs TSR", xKey: "tsr", xLabel: "Tip Speed Ratio (λ)",
      yLabel: "Cp", type: "line",
      series: [{ key: "Cp", label: `Cp (${blades} blades)`, color: "#34d399" }],
      data: tsrData, currentX: +currentTSR.toFixed(2),
      annotation: `Peak Cp: ${Math.max(...tsrData.map(d => d.Cp)).toFixed(3)} (Prandtl tip-loss applied)`,
    },
  ];
}
