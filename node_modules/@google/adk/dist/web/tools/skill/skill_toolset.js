...
let SkillToolset = class extends BaseToolset {
  ...
  async getOrFetchSkill(name, invocationId) {
    ...
    try {
      ...
      logger.warn("Failed to fetch skill '".concat(name, "' from registry: ").concat(e));
      throw e;
    }
  }
  ...
  async resolveAdditionalTools(context) {
    ...
    const cacheKey = "".concat(agentName, ":").concat(activatedSkills.join(","));
    ...
    for (const toolUnion of this.additionalTools) {
      if (toolUnion instanceof BaseTool) {
        if (candidateTools[toolUnion.name]) {
          throw new Error("Duplicate tool name: ".concat(toolUnion.name));
        }
        ...
        for (const t of tsTools) {
          if (candidateTools[t.name]) {
            throw new Error("Duplicate tool name: ".concat(t.name));
          }
