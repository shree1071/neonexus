...
let RunSkillScriptTool = class extends BaseTool {
  ...
  async runAsync({
    ...
    try {
      ...
      return {
        error: "Failed to fetch skill '".concat(skillName, "' from registry: ").concat(e.message || e),
    ...
    if (!script) {
      return {
        error: "Script '".concat(scriptPath, "' not found in skill '").concat(skillName, "'."),
    ...
    try {
      ...
      return {
        error: "Failed to execute script '".concat(scriptPath, "': ").concat(e.message),
...
function buildWrapperCode(scriptPath, language) {
  switch (language) {
    case CodeExecutionLanguage.JAVASCRIPT:
      return "require('./".concat(scriptPath, "');");
    case CodeExecutionLanguage.TYPESCRIPT:
      return "require('ts-node/register');\nrequire('./".concat(scriptPath, "');");
    case CodeExecutionLanguage.PYTHON:
      return "import runpy\nrunpy.run_path('./".concat(scriptPath, "', run_name='__main__')");
    case CodeExecutionLanguage.SHELL:
      return "source ./".concat(scriptPath, ' "$@"');
    case CodeExecutionLanguage.POWERSHELL:
      return "& .\\".concat(scriptPath.replace(/\//g, "\\\\"), " $args");
    case CodeExecutionLanguage.WINDOWS_CMD:
      return "call .\\".concat(scriptPath.replace(/\//g, "\\\\"), " %*");
    default:
      throw new Error("Unsupported wrapper language: ".concat(language));
  }
}
function getSkillResourceFiles(skill) {
  ...
  for (const resourceType of ["references", "assets", "scripts"]) {
    ...
    for (const resourceName of Object.keys(resources)) {
      ...
      files.push({
        name: "".concat(resourceType, "/").concat(resourceName),
