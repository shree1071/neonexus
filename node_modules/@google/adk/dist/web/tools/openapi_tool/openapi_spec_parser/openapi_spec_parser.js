  ...
  const recursiveResolve = (obj, currentDoc, seenRefs = /* @__PURE__ */ new Set()) => {
    ...
    if ("$ref" in objRecord && typeof objRecord["$ref"] === "string") {
      ...
      if (seenRefs.has(refString) && !resolvedCache.has(refString)) {
        const copy = __spreadValues({}, objRecord);
        delete copy["$ref"];
        return copy;
      }
...
function resolveRef(refString, currentDoc) {
  ...
  if (parts[0] !== "#") {
    throw new Error("External references not supported: ".concat(refString));
  }
  ...
  for (const [path, pathItem] of Object.entries(spec.paths || {})) {
    ...
    for (const method of methods) {
      ...
      if (!operation.operationId) {
        operation.operationId = "".concat(method, "_").concat(path.replace(/[^a-zA-Z0-9]/g, "_"));
      }
