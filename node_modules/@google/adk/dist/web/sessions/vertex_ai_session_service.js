...
function quoteFilterLiteral(value) {
...
let VertexAiSessionService = class extends BaseSessionService {
  ...
  async createSession({
    ...
    const filteredState = state ? trimTempState(state) : void 0;
    let apiResponse = await this.sessions.createInternal({
      name: "reasoningEngines/".concat(reasoningEngineId),
      userId,
      config: __spreadValues(__spreadValues({}, filteredState ? { sessionState: filteredState } : {}), sessionId ? { sessionId } : {})
    });
  ...
  async listSessions({
    ...
    do {
      const response = await this.sessions.listInternal({
        name: "reasoningEngines/".concat(reasoningEngineId),
        config: __spreadValues(__spreadValues({}, userId ? { filter: "user_id=".concat(quoteFilterLiteral(userId)) } : {}), pageToken ? { pageToken } : {})
      });
