...
async function loadDir(directoryPath) {
  ...
  try {
    ...
    logger.warn(
      "Failed to load directory '".concat(directoryPath, "': ").concat(e.message)
    );
  }
  return files;
}
function parseSkillMdContent(content) {
  ...
  try {
    ...
    throw new Error("Invalid YAML in frontmatter: ".concat(e.message));
  }
}
...
async function validateSkillDir(skillDir) {
  ...
  try {
    ...
    if (unknown.length > 0) {
      problems.push(
        "Unknown frontmatter fields: [".concat(unknown.sort().join(", "), "]")
      );
    }
    const dirName = path.basename(resolvedDir);
    if (dirName !== skill.frontmatter.name) {
      problems.push(
        "Skill name '".concat(skill.frontmatter.name, "' does not match directory name '").concat(dirName, "'.")
      );
    }
  } catch (e) {
...
async function loadSkillFile(skillDir) {
  ...
  try {
    ...
    logger.warn(
      "Failed to load directory '".concat(skillDir, "': ").concat(e.message)
    );
  }
  if (!skillMdPath) {
    throw new Error(
      "SKILL.md (or any case variation like skill.md) not found in '".concat(skillDir, "'.")
    );
  }
  ...
  if (dirName !== frontmatter.name) {
    throw new Error(
      "Skill name '".concat(frontmatter.name, "' does not match directory name '").concat(dirName, "'.")
    );
  }
  return {
    frontmatter,
...
async function loadAllSkillsInDir(skillsBasePath) {
  ...
  async function scanDir(currentDir) {
    ...
    if (isSkillDir) {
      try {
        ...
        logger.warn("Skipping invalid skill in '".concat(currentDir, "':"), e);
      }
    } else {
      for (const entry of entries) {
        if (entry.isDirectory()) {
  ...
  try {
    ...
    logger.warn("Skills base path '".concat(skillsBasePath, "' is not a directory."), e);
  }
  return skills;
}
function loadSkillFromZipBuffer(zipBuffer) {
  ...
  function loadZipDir(prefix) {
    const res = {};
    const normPrefix = prefix.endsWith("/") ? prefix : "".concat(prefix, "/");
    for (const entry of entries) {
      if (entry.isDirectory) continue;
