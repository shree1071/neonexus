...
class Runner {
  ...
  async saveArtifacts(invocationId, userId, sessionId, message) {
    ...
    for (let i = 0; i < message.parts.length; i++) {
      ...
      await this.artifactService.saveArtifact(__spreadProps(__spreadValues({}, sessionKey), {
        filename: fileName,
        artifact: part
      }));
...
function determineAgentForResumption(session, rootAgent, resumabilityConfig) {
  ...
  if (event && event.author && isResumable) {
    ...
    logger.warn(
      "Function response from an unknown agent: ".concat(event.author, ", event id: ").concat(event.id)
    );
  }
