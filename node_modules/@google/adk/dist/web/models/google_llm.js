...
class Gemini extends (_b = BaseLlm, _a = GEMINI_MODEL_SYMBOL, _b) {
  /**
   ...
   */
  generateContentAsync(llmRequest, stream = false, abortSignal) {
    return __asyncGenerator(this, null, function* () {
      var _a2, _b2, _c;
      if (this.useInteractionsApi) {
        yield* __yieldStar(generateContentViaInteractions(this.apiClient, llmRequest, stream));
        return;
      }
      this.preprocessRequest(llmRequest);
      this.maybeAppendUserContent(llmRequest);
      logger.info(
        "Sending out request, model: ".concat((_a2 = llmRequest.model) != null ? _a2 : this.model, ", backend: ").concat(this.apiBackend, ", stream: ").concat(stream)
      );
      ...
      if (stream) {
        ...
        try {
          for (var iter2 = __forAwait(streamResult), more2, temp2, error2; more2 = !(temp2 = yield new __await(iter2.next())).done; more2 = false) {
            const response = temp2.value;
            try {
              for (var iter = __forAwait(aggregator.processResponse(response)), more, temp, error; more = !(temp = yield new __await(iter.next())).done; more = false) {
