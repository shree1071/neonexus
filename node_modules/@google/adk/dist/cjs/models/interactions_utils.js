...
var interactions_utils_exports = {};
__export(interactions_utils_exports, {
  buildGenerationConfig: () => buildGenerationConfig,
  convertContentToSteps: () => convertContentToSteps,
  convertInteractionEventToLlmResponse: () => convertInteractionEventToLlmResponse,
  convertInteractionToLlmResponse: () => convertInteractionToLlmResponse,
  convertStepToParts: () => convertStepToParts,
  convertToolsConfigToInteractionsFormat: () => convertToolsConfigToInteractionsFormat,
  extractSystemInstruction: () => extractSystemInstruction,
  generateContentViaInteractions: () => generateContentViaInteractions,
  getLatestModelParts: () => getLatestModelParts,
  getLatestUserContents: () => getLatestUserContents
});
module.exports = __toCommonJS(interactions_utils_exports);
    ...
    if (content.parts) {
      for (const part of content.parts) {
        if (part.functionResponse) {
          ...
          const isError = part.codeExecutionResult.outcome === import_genai.Outcome.OUTCOME_FAILED || part.codeExecutionResult.outcome === import_genai.Outcome.OUTCOME_DEADLINE_EXCEEDED;
          steps.push({
      ...
      return [
        {
          codeExecutionResult: {
            ...
            outcome: codeExecutionResultStep.is_error ? import_genai.Outcome.OUTCOME_FAILED : import_genai.Outcome.OUTCOME_OK
          }
        }
      ];
    }
...
async function* generateContentViaInteractions(apiClient, llmRequest, stream) {
  ...
  if (stream) {
    ...
    import_logger.logger.info("Interaction response received from the model.");
