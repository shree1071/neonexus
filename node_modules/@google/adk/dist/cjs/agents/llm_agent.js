...
var import_interactions_request_processor = require("./processors/interactions_request_processor.js");
...
class LlmAgent extends (_b = import_base_agent.BaseAgent, _a = LLM_AGENT_SIGNATURE_SYMBOL, _b) {
  constructor(config) {
    ...
    this.requestProcessors = (_g = config.requestProcessors) != null ? _g : [
      ...
      import_auth_preprocessor.AUTH_PREPROCESSOR,
      ...
      import_interactions_request_processor.INTERACTIONS_REQUEST_PROCESSOR,
  ...
  async *runAsyncImpl(context2) {
    ...
    while (true) {
      ...
      for await (const event of this.runOneStepAsync(context2)) {
        ...
        lastEvent = event;
        if ((0, import_event.getFunctionCalls)(event).length > 0 || (0, import_event.getFunctionResponses)(event).length > 0) {
      ...
      if ((0, import_event.isFinalResponse)(lastEvent) && !(isEmptyMetadataEvent && stepHadToolCalls)) {
        break;
      }
  ...
  async *runOneStepAsync(invocationContext) {
    ...
    if (this.outputSchema && allTools.length > 0) {
      const setModelResponseTool = new import_function_tool.FunctionTool({
  ...
  async *postprocess(invocationContext, llmRequest, llmResponse, modelResponseEvent) {
    ...
    if ((!llmResponse.content || ((_b2 = llmResponse.content.parts) == null ? void 0 : _b2.length) === 0) && !llmResponse.errorCode && !llmResponse.interrupted) {
      return;
    }
    ...
    yield mergedEvent;
    if (!((_c = (0, import_event.getFunctionCalls)(mergedEvent)) == null ? void 0 : _c.length)) {
      return;
    }
