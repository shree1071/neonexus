...
__export(content_processor_utils_exports, {
  ...
  mergeFunctionResponseEvents: () => mergeFunctionResponseEvents,
...
function removeClientFunctionCallId(content) {
  var _a, _b, _c, _d, _e;
  for (const part of (_a = content == null ? void 0 : content.parts) != null ? _a : []) {
    if ((_c = (_b = part.functionCall) == null ? void 0 : _b.id) == null ? void 0 : _c.startsWith(import_functions.AF_FUNCTION_CALL_ID_PREFIX)) {
      part.functionCall.id = void 0;
    }
    if ((_e = (_d = part.functionResponse) == null ? void 0 : _d.id) == null ? void 0 : _e.startsWith(import_functions.AF_FUNCTION_CALL_ID_PREFIX)) {
...
function convertForeignEvent(event) {
  ...
  for (const part of event.content.parts) {
    if (part.text && !part.thought) {
      ...
      (_f = content.parts) == null ? void 0 : _f.push((0, import_lodash_es.cloneDeep)(part));
    }
  }
...
function mergeFunctionResponseEvents(events) {
  ...
  const mergedEvent = (0, import_event.createEvent)({
    ...events[0],
    content: events[0].content ? (0, import_lodash_es.cloneDeep)(events[0].content) : void 0
  });
  ...
  for (const event of events.slice(1)) {
    ...
    for (const part of event.content.parts) {
      const clonedPart = (0, import_lodash_es.cloneDeep)(part);
      if (clonedPart.functionResponse && clonedPart.functionResponse.id) {
        ...
        if (functionCallId in partIndicesInMergedEvent) {
          partsInMergedEvent[partIndicesInMergedEvent[functionCallId]] = clonedPart;
        } else {
...
function rearrangeEventsForLatestFunctionResponse(events) {
  ...
  const functionResponsesIds = new Set(
