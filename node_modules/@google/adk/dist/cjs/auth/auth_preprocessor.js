  ...
  for (const event of events) {
    ...
    for (const functionCall of eventFunctionCalls) {
      if (functionCall.id && authFcIds.has(functionCall.id) && functionCall.name === import_functions.REQUEST_EUC_FUNCTION_CALL_NAME) {
        const args = (0, import_case_utils.camelCaseKeys)(functionCall.args);
        const authConfig = args == null ? void 0 : args.authConfig;
        if (authConfig) {
    ...
    for (const event of events) {
      ...
      for (const functionCall of eventFunctionCalls) {
        if (functionCall.id === fcId && functionCall.name === import_functions.REQUEST_EUC_FUNCTION_CALL_NAME) {
          const args = (0, import_case_utils.camelCaseKeys)(
            functionCall.args
          );
          const functionCallId = args == null ? void 0 : args.functionCallId;
          if (functionCallId) {
