...
class EntityComparator {
    ...
    getPkGetter(meta) {
        ...
        context.set('isEntityOrRef', (val) => Utils_1.Utils.isEntity(val, true));
    ...
    getInlineEmbeddedNullLines(meta, parentName, padding, dataKey, useEmbeddedKey) {
        return meta.props.filter(p => p.embedded?.[0] === parentName
            && (p.object || (p.persist !== false))).flatMap(childProp => {
            const childDataKey = useEmbeddedKey ? dataKey + this.wrap(childProp.embedded[1]) : this.wrap(childProp.name);
            if (childProp.kind === enums_1.ReferenceKind.EMBEDDED && !childProp.object) {
