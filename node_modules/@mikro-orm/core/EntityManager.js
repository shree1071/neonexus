...
class EntityManager {
    ...
    async refresh(entity, options = {}) {
        ...
        for (const e of fork.unitOfWork.getIdentityMap()) {
            ...
            const data = (0, entity_1.helper)(e).serialize({ ignoreSerializers: true, includeHidden: true, convertCustomTypes: false });
            em.config.getHydrator(this.metadata).hydrate(ref, (0, entity_1.helper)(ref).__meta, data, em.entityFactory, 'full', false, false);
            utils_1.Utils.merge((0, entity_1.helper)(ref).__originalEntityData, this.comparator.prepareEntity(e));
            found ||= ref === entity;
        }
        if (!found) {
            const data = (0, entity_1.helper)(reloaded).serialize({ ignoreSerializers: true, includeHidden: true, convertCustomTypes: false });
    ...
    async upsertMany(entityNameOrEntity, data, options = {}) {
        ...
        if (options.onConflictAction === 'ignore' || (!res.rows?.length && loadPK.size > 0) || reloadFields) {
            ...
            const data2 = await this.driver.find(meta.className, where, {
                fields: returning.concat(...add).concat(...(Array.isArray(uniqueFields) ? uniqueFields : [])).concat(...(options.onConflictMergeFields ?? [])),
                ctx: em.transactionContext,
                convertCustomTypes: true,
                connectionType: 'write',
                schema: options.schema,
    ...
    map(entityName, result, options = {}) {
        ...
        Object.keys(data).forEach(k => {
            ...
            if (prop?.kind === enums_1.ReferenceKind.SCALAR && enums_1.SCALAR_TYPES.includes(prop.runtimeType) && !prop.customType && (prop.setter || !prop.getter)) {
    ...
    async preparePopulate(entityName, options, validate = true) {
        ...
        const populateArray = Array.isArray(options.populate) ? options.populate : undefined;
